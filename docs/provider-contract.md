...
| --- | --- | --- | --- |
| Codex | `codex app-server --stdio`, JSON-RPC/JSONL | 0.145.x–0.147.x | provider thread resume |
...
Discovery resolves executables only from explicit `PATH` entries, probes versions without a shell, and records capability/health snapshots locally. Tested ranges remain internal compatibility metadata; the UI reports actionable startup or protocol failures instead of presenting maintenance ranges as provider errors. The runtime blocks unsupported plan, effort, cancellation, or persistence controls before a provider run starts.
